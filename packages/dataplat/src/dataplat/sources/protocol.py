...
``ARCHITECTURE.md`` Q4.3's original sketch attached two extra schema- and
profile-describing attributes to ``RecordStream`` and an ``inspect(ctx)``
method to ``Source`` that returns the profile one. Neither of those two
attributes' types existed anywhere in this codebase before Phase 6's
detection engine landed them (03-CONTEXT.md's explicit "Out of scope" list
named this exact gap). Phase 6 has now landed ``inspect()`` below, returning
``dataplat.models.profile.CsvProfile`` — this file is this seam's final
shape, not its Phase-3 placeholder.

``csv_processor.source`` (plan 03-08; ``CsvSource.inspect()``/``open()``
wired for real in plan 06-14) is the first concrete ``Source``.
"""
...
class Source(Protocol):
    ...
    def inspect(self, ctx: PipelineContext) -> CsvProfile:
        """Run this source's detection once, returning everything ``open()``/chunking need.

        Args:
            ctx: The current pipeline context.

        Returns:
            The detected (or contract-resolved) profile.
        """
        ...
