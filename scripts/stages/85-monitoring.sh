...
# Quick task 260824-ayw: this stage script's job is now only to decide
# WHETHER to install (the PROFILE guard above it), never HOW -- the actual
# helm_install/wait_for_* logic now lives in scripts/monitoring-install.sh,
# shared with the new CI staggered path (Makefile's observability-verify-ci)
# so the two paths can never drift into two different install shapes.
"${repo_root}/scripts/monitoring-install.sh"
