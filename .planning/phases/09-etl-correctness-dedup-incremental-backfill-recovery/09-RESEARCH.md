...
1. **What exact `--from-date`/`--to-date` window (and how many minutes/DagRuns) should the live 2-year backfill proof use?**
   ...
   - **RESOLVED BY PLANNING:** deferred to execution time as an empirical, live-measured step, exactly as recommended above — planning cannot pre-determine real cluster throughput. `09-11-PLAN.md` Task 1 (`test_dry_run_sizing_reports_reasonable_dagrun_count`, `test_pilot_window_drains_without_cpu_starvation`) implements the `--dry-run` sizing check and the pilot-then-degrade (`--max-active-runs 3` → `1`) sequence verbatim.
...
2. **Does the 2-year fixture corpus generator already exist, or is it new work?**
   ...
   - **RESOLVED BY PLANNING:** `tools/corpus/generators.py` was read in full during planning — its `Manifest`/`Fixture` model is declaration-per-fixed-file (one manifest entry, one byte-exact output) with no "N dated files, one per day" concept, so it CANNOT express D-10's combined requirement without new code. `09-05-PLAN.md` builds a purpose-built, ADR-0005-R1-R10-compliant generator, `tools/corpus/dated_series.py::generate_dated_series`, reusing `generators.py`'s determinism primitives (`stream_for`, `_pick_renderer`-equivalents) but NOT its `Manifest`/`Fixture`/`generate_corpus` machinery — confirmed new work, not a manifest addition.
