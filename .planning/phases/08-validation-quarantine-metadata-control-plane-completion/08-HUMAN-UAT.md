...
updated: 2026-08-17T21:15:00Z
---

## Current Test

[testing paused — both items now have DEFINITIVE, live-confirmed results. Gap 1 (E2E deploy / ingest starvation): still open, deliberately-deferred infra decision (kind cluster node CPU budget, physical host ceiling) — no code fix possible, tracked in STATE.md's Blockers. Gap 2 (backfill re-drive): the retry-timing MECHANISM is now fully fixed and live-proven (plan 08-15 + two live-driven refinements, commits 441a51a/368c83c) — no more timing errors, clean backfill re-execution end to end. But with the mechanism working, the test finally ran all the way through and CONFIRMED, for the first time with live evidence, a real `dataplat` architecture gap: a corrected file's backfill re-drive does NOT flip the original PENDING reject to REDRIVEN, because `discover_files`'s `batch_key` is a pure function of `content_sha256` (a content-differing correction always gets a new batch_id) while `resolve_rejected_records_for_batch` scopes strictly by `batch_id`. This is Rule-4-territory (a real design decision, not a quick fix) — see deferred-items.md and .planning/debug/resolved/backfill-does-not-redrive-rejected-row.md for full detail and a candidate fix direction.]
