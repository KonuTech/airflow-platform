...
1. **Which Airflow ServiceAccount(s) must the `airflow` Vault role bind to?**
...
2. **Should Vault's listener run with TLS enabled for this project's local-dev deployment?**
...
3. **`vault` CLI binary vs. pure-`hvac`-Python bootstrap — which does the planner prefer?**
