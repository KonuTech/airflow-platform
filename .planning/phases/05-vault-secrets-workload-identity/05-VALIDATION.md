...
| T2 | 05-06 | 6 | SEC-13 | T-05-16 | The fix is proven against the real live cluster from a genuinely empty Vault (scoped Vault-release-and-PVC reinstall, namespace `vault` only), not merely by code review or mocks — `make vault-verify`'s full suite and a real DAG run both succeed using the freshly-generated credentials | e2e (cluster) | `pytest tests/e2e/vault -q -m cluster` | ⚠️ partial — 15/16 passed live (2026-08-14) against a genuinely fresh, empty Vault; `make vault-bootstrap` printed "created" (not "already present") for all 3 previously-broken paths, proving CR-01. The 1 failure (`test_airflow_backend.py::test_dag_still_resolves_its_connection_and_runs`) is a pre-existing, unrelated Airflow `KubernetesExecutor` scheduling stall (`max_active_runs: 1` backlog + watch-replay loop), not a credential fault — see `docs/secrets-architecture.md` §6 for the full account. The real-DAG-run clause of this row is therefore **not yet independently satisfied**; tracked as an open follow-up outside 05-06's scope. |
...
| Full reproducibility from a completely fresh cluster (SEC-13's other half) | SEC-13 | **Partially superseded by gap-closure plan 05-06, with a partial live result.** A full `kind delete cluster` + recreate cycle inside a pytest run remains disproportionately slow for a per-wave gate, so it is still not run automatically on every wave. But this is no longer purely manual: `tests/unit/test_vault_bootstrap.py` (plan 05-06, Task 1) offline-proves the empty-Vault credential-sourcing code path with mocks (7/7 passing, independently re-confirmed), and plan 05-06's Task 2 performed one real live-cluster proof (a scoped Vault-release-and-PVC reinstall — the same empty-KV-store precondition a full cluster rebuild produces, without the full teardown cost). The credential-sourcing fix itself is proven: `make vault-bootstrap` printed "created" (not "already present") for all 3 previously-broken paths against a genuinely empty Vault, and 15/16 `tests/e2e/vault` tests passed, including every test exercising credential *function* (positive/negative auth, audit log, rotation, idempotent-rerun). What remains unproven is the *real-DAG-run* clause specifically: the 16th test timed out due to a separate, pre-existing Airflow `KubernetesExecutor` scheduling stall (`max_active_runs: 1` backlog run + a Kubernetes-watch replay loop), not a credential fault. Two scoped remediation attempts (clearing the stuck task; deleting 3 orphaned pod objects) did not resolve it. This is tracked as an open, separate issue. | The scoped proof (plan 05-06, Task 2, 2026-08-14): delete PVCs `data-vault-0`/`audit-vault-0` and pod `vault-0` in namespace `vault` only, let the StatefulSet reconcile a fresh empty Vault, then run `make vault-unseal && make vault-bootstrap && make vault-verify`. 15/16 passed; the failing test and its unrelated root cause are detailed in `docs/secrets-architecture.md` §6. Re-running just `pytest tests/e2e/vault/test_airflow_backend.py -m cluster` once the Airflow scheduler issue is separately resolved would close this out fully. The full literal `make cluster-down && make cluster-up && make vault-unseal && make vault-bootstrap && make vault-verify` sequence from an actual clean checkout remains unexercised by this project's own automation — a residual, honestly-disclosed limit, not a hidden gap. |
...
regression guard) is fully green. Task 2 (live-cluster proof) is partial: the credential-sourcing fix
itself is proven live (15/16 `tests/e2e/vault` passing, `make vault-bootstrap` confirmed creating all
3 previously-broken paths against a genuinely empty Vault), but the real-DAG-run clause remains blocked
by a separate, pre-existing Airflow scheduler fault unrelated to Vault/credentials — not resolved as
part of this plan. See the T2/05-06 row above and `docs/secrets-architecture.md` §6 for the full account.
</content>
