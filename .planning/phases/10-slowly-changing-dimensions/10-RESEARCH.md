...
1. **SCD-01 (Type 0) has no assigned column in D-02's locked column-treatment table.**
   ...
   - **RESOLVED:** D-13 (CONTEXT.md) designates `signup_country` as the dedicated Type-0 business column -- the concrete proof this question asked for, not the `customer_id`-immutability fallback.
...
2. **`timestamp` vs `check` change-detection strategy (dbt vocabulary, Claude's discretion per CONTEXT.md) — which one actually governs `customers.yaml`'s hash comparison?**
   ...
   - **RESOLVED:** The `check` strategy (normalized-hash comparison, not raw `event_ts` advance) was adopted as the phase recommendation and is implemented via plan 10-01, `ColumnContract.scd_type` dispatch -- `event_ts` governs ordering/boundary placement only, the hash governs whether a boundary is drawn at all.
...
3. **Does the 2-year corpus's existing deliberate schema-version change (Phase 9 D-10) interact correctly with SCD2 versioning?**
   ...
   - **RESOLVED:** Verified in plan 10-02 Task 2, Test 8 -- recompute compares NORMALIZED VALUES under a single consistent hash recipe, never raw stored `_record_hash` values across differing `hash_version`s, so the `loyalty_tier` schema-change boundary cannot be misread as a `name`/`country` attribute change.
