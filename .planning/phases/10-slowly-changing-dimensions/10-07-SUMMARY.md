...
plan: 07
subsystem: testing
tags: [scd2, live-e2e, kind-cluster, backfill, circuit-breaker, cluster-pacing]
...
duration: ~6h (Task 1 authored in a prior, crashed executor session; this session covered live re-verification, root-cause investigation of a live pacing failure, and a timeout re-tuning fix)
completed: 2026-08-21
---

# Phase 10 Plan 07: Live 2-Year Sweep -- SCD Publisher Proof + D-06 Mass-Delete Circuit Breaker Summary

**The live 2-year sweep's extended corpus and all three of this plan's tasks (SCD version-boundary/late-correction/DELETE-invalidate proofs, D-12 version-count idempotency + Pattern-4 corruption-check rewrite, and D-06's mass-delete circuit-breaker live trip) are code-complete and committed, with two real live bugs found and fixed along the way -- but a full, clean, all-6-tests-green live run could not be completed within this session due to pre-existing, cluster-wide zombie-backfill contention that this session's own investigation traced to a real gap in this project's own stale-backfill remediation practice (see Verification Status).**
...
**Total deviations:** 3 (2 from the prior session -- 1 Rule 1 bug, 1 Rule 4 architectural change explicitly user-reviewed before implementation; 1 from this session -- Rule 1 bug/miscalibration). No scope creep beyond what each finding required.
...
- **A full, clean, all-6-tests-green live run of `pytest tests/e2e/slice/test_backfill_2year_sweep.py -q -m cluster` could not be completed within this session.** This session ran the module's first two tests (`test_dry_run_sizing_reports_reasonable_dagrun_count`, `test_pilot_window_drains_without_cpu_starvation`) twice: the first attempt failed exactly as the orchestrator's hand-off described (a 600s/900s timeout too tight for the now-larger corpus); after the `98f7330` fix, a second live re-run was attempted, but as of this session's end `test_pilot_window_drains_without_cpu_starvation`'s own new backfill (id 43) was still in progress after ~2h14m wall-clock (started ~19:56 UTC, still running at 22:10 UTC), its `publish` task itself retrying (try 3 of 4) against a live cluster independently confirmed to be carrying multiple pre-existing, still-actively-retrying zombie backfill DagRuns (39-42) from before this session began. This session's own code review of Tasks 1/2/3's test logic (detailed above, cross-checked line-by-line against the plan's own acceptance criteria) gives HIGH confidence the code is correct; the live pass/fail status of the FULL module against a clean cluster remains unconfirmed by this session.
- **Recommendation for the orchestrator/user:** before any future live re-verification attempt of this module (or any other live e2e suite sharing this cluster), actively terminate backfills 39-42's own still-running `dag_run`s (not just their `backfill` row's `completed_at` bookkeeping) -- e.g. `airflow dags trigger`-adjacent state-clearing or a direct `dag_run.state = 'failed'` update followed by killing any in-flight task pods -- since force-completing the `backfill` row alone does not stop them, and they will continue to intermittently contend for this DAG's shared `max_active_tis_per_dag=1`/`max_active_runs=1` concurrency slots indefinitely.
...
- Two real, live-discovered SCD-publish-path bugs (OOM, unscoped mass-delete detection) are fixed and committed.
- **Blocker for full sign-off:** a clean, complete live run of the whole module (`pytest tests/e2e/slice/test_backfill_2year_sweep.py -q -m cluster`) is still needed to close this plan out with full confidence -- recommended only after the zombie-backfill cleanup noted above, to get a live pass/fail signal uncontaminated by pre-existing cluster state this plan's own code did not cause.
...
- FOUND commit: 98f7330 (this session's timeout re-tune)
