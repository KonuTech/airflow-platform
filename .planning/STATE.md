...
- **PENDING MANUAL ACTIVATION (2026-08-17, quick task `260817-oqy`, `.planning/quick/260817-oqy-raise-kind-cluster-memory-budget-28gb-ws/`):** the structural node-CPU-budget question (flagged deferred since 2026-08-16, and confirmed still live even after `260817-mvp`'s `integrity_gate` cap — a re-run of the phase-08 E2E slice tests still hit CPU/memory starvation, now on `ingest` instead) has a memory-axis fix staged but **not yet active**: `/mnt/c/Users/admin/.wslconfig`'s WSL2 memory cap raised 24GB→28GB, and `kind/cluster.yaml`'s three `KubeletConfiguration` blocks recomputed (`systemReserved.memory` 9Gi→11Gi, `kubeReserved.memory` 8Gi→9.5Gi) using the file's own documented fair-share formula — commit `811438b`. CPU stays at 3 CPU/node (9 CPU cluster-wide) since the host is a physical 12-core ceiling; this is a memory-only improvement (~6.3Gi→~7.5Gi allocatable/node), not a full fix. **Still required to take effect:** (1) run `wsl --shutdown` from Windows PowerShell (not from inside WSL) + restart Docker Desktop, (2) confirm the new cap (`free -h` should show ~28Gi), (3) `kind delete cluster` + recreate (destroys current Postgres/MinIO/Vault data — user already accepted this), (4) full stack redeploy (Helm charts, Vault bootstrap/unseal, migrations, image builds). None of steps 1-4 have been run yet.
