...
## Retry-Timing Fix Applied and Re-Verified Live (2026-08-17T20:03-20:22Z)

Fix implemented (quick task `260817-umv`, commit `441a51a`): `_fetch_latest_backfill_dag_run_row` now returns `(row_found, exception_reason, completed_at)`; `_wait_for_backfill_dag_run_row`'s settle loop gates on `completed_at IS NOT NULL`, not just row-appearance; `_BACKFILL_ROW_SETTLE_TIMEOUT_SECONDS` raised `15.0` → `45.0`. Static verification clean (ruff, mypy, collect-only).

Re-ran `pytest tests/e2e/slice/test_backfill_reentry.py -x -m cluster` live, twice more. **Neither attempt ever reached the backfill retry code this fix targets** — both failed identically, at `poll_file_discovered` for the ORIGINAL file upload, before any backfill logic runs at all.

**Root cause, confirmed via live SQL both times:** `csv_ingest_customers`'s `*/1 * * * *` cron (D-03) re-lists the ENTIRE `customers/*.csv` S3 prefix on EVERY firing (not incrementally), and `integrity_gate` (capped `max_active_tis_per_dag=3` since quick task `260817-mvp`) re-evaluates every matched key every single cycle, including files already `PROCESSED` long ago. Live-confirmed: **91 files** currently sit under `raw/customers/` (MinIO `ls` count). One full DagRun (`scheduled__2026-08-17T20:06:00`) took **~11 minutes** (20:07:17 → 20:18:11) to clear a 54-file `integrity_gate` backlog at the 3-concurrent cap; the VERY NEXT DagRun (`scheduled__2026-08-17T20:17:00`) immediately started re-processing the same ~55 files again (21 done, 3 queued, 31 still scheduled after 3+ minutes), because `max_active_runs=1` means only one DagRun total is ever in flight and each one restarts the full re-scan.

This is NOT a one-time drain-the-backlog situation — it is a **structural, permanently-recurring** cost: every cron cycle re-pays the full backlog re-evaluation, so a freshly-uploaded test file's `discover` step can never realistically land inside this test's 180s `_DISCOVERY_TIMEOUT_SECONDS` window as long as ~90 historical files remain in the prefix. The `260817-mvp` concurrency cap fixed cluster-wide starvation (a real, separate win, still valid) but did not and could not fix this — capping concurrency makes each cycle SLOWER per-file-processed, trading starvation-of-other-DAGs for guaranteed-multi-minute-cycles-of-this-DAG.

**The retry-timing fix (`441a51a`) is unverified live, not disproven** — it was never exercised because discovery itself never completed in either attempt. Static verification (code review + ruff/mypy/collect-only) is the only evidence for it so far.

**Actionable lever, not yet taken (deferred-items.md's own suggestion, now confirmed necessary, not just "worth considering"):** archive or delete the ~90 historical E2E-test fixture files under `raw/customers/*.csv` that are already `PROCESSED` (safe under this platform's raw-immutability principle, D-63 — archiving is not the same as deleting evidence of a batch that was already durably recorded in `meta.files`/`meta.ingestion_runs`; the historical processing record survives independent of the raw object's continued presence in the bucket). This is the only currently-available lever that would let a fresh `discover` land inside a reasonable timeout on this specific, long-lived demo cluster, short of redesigning `list_matched_keys` to be incremental (a real architecture change, out of scope for a quick fix).

## Backlog Cleared, Second Retry-Timing Refinement, and the Definitive Result (2026-08-17T20:35-21:15Z)

**Backlog cleared:** user approved and ran a bulk `delete_objects` against `raw/customers/*` (54 live objects, all synthetic `e2e-*`/`ingest-demo-*` fixtures) — 54/54 deleted cleanly, prefix confirmed empty afterward.

**Second refinement to `_wait_for_backfill_dag_run_row` (commit `368c83c`):** the completion-gated wait added in `441a51a` was gating unconditionally, even when `exception_reason` already showed a genuine success (non-`"in flight"`). But the caller only fires another `airflow backfill create` when the return value IS `"in flight"` — so waiting for `completed_at` on a real success serves no purpose and can itself time out, since a genuine full DAG re-execution (clear + rerun the downstream task graph) legitimately takes longer than the `"in flight"` bookkeeping-only case the 45s timeout was sized against. Fixed: `completed_at` is now only required when `exception_reason == "in flight"`; any other value returns immediately on `row_found`.

**Result: the test progressed further than ever before, past EVERY step except the final assertion.** `discover`, `ingest` (SUCCEEDED), the PENDING-reject checks, `dag_run` identity resolution, the real `airflow backfill create` re-execution (`clear_number` advanced, state reached `success` — no retry-timing errors at all this run, the fix worked cleanly), and the corrected row publishing to `normalized.customers` (`name='Corrected Name'`) all passed. It failed ONLY at `_assert_row_resolved`: the original `rejected_record_id`'s `resolution_type` is still `'PENDING'`, not `'REDRIVEN'`.

**This is the batch_key/content_sha256 architecture concern, CONFIRMED live for the first time — not refuted, not merely theoretical.** Direct SQL: the original reject (`rejected_record_id=8`) is scoped to `batch_id=43120`; the corrected file's own ingestion run is `run_id=43121`, under a **different** `batch_id`. Since `discover_files`'s `batch_key` is a pure function of `content_sha256` (deferred-items.md, "From plan 08-14"), and `resolve_rejected_records_for_batch` scopes strictly by `batch_id`, the corrected (content-differing) file discovers under a brand-new `batch_id` that the resolve call for the new run never touches the OLD batch's PENDING row under. The mechanism (Airflow backfill re-execution, the retry-timing fix, the whole DAG-level plumbing) all now work correctly and are proven live — this is a genuine `dataplat` architecture gap, exactly as originally flagged, not a test-robustness or infra issue.

**Status: VALID-08's re-drive path does NOT work end-to-end as currently designed.** This needs a real architecture decision (deferred-items.md's own words: "Rule 4 territory: content-addressed batching vs. row-level correction is a design decision, not a one-line bug fix") — e.g., resolving PENDING rejects by business-key + dataset rather than strictly by `batch_id`, or making `resolve_rejected_records_for_batch` also consider the newly-discovered file's *content* relationship to the original reject. Out of scope for further quick-task iteration; needs a dedicated planning/design pass.
