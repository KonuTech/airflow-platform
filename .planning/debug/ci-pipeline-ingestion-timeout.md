...
parallel debug thread, explicitly out of scope here):
  ...
  live_verification_state: "RECORDED 2026-08-29T06:41:20Z: ROUND 22 pushed as code commit
      ...
      --stat` before staging."
  next_action: "SUPERSEDED -- see ROUND 22 OUTCOME below."

ROUND 22 OUTCOME (2026-08-29, post-run analysis of run 33239055603, headSha 2009065 -- CURRENT STATE):
  run: "e2e-full.yml 33239055603, headSha 2009065, conclusion FAILURE, job 06:41:00Z->06:44:24Z =
      3m24s TOTAL. This is NOT a suite-level failure -- the job died inside step 7 ('Bring up the
      ephemeral kind cluster (CI profile)', i.e. `make cluster-up`), before the E2E test suite
      (step 13) ever ran (steps 9-13, 15-16 all show status=skipped). Log saved as scratchpad
      round22-job.log (1,241 lines -- two orders of magnitude shorter than every prior round's
      ~10,000-17,000-line suite log, itself diagnostic of an early infra-stage death, confirmed
      before reading a single line of content)."
  root_cause_this_run: "Direct log read, stage `60-minio.sh` (job log lines 769-773): `scripts/
      minio-credentials.sh ensure`'s `_create_secret` helper builds the `minio-root` Secret's
      `stringData` block as UNQUOTED YAML scalars piped to `kubectl apply -f -`. `rootUser` is
      generated by `_random_hex 8` (8 bytes = 16 lowercase-hex characters, `openssl rand -hex 8`).
      This specific run's randomly-generated `rootUser` value happened to consist ENTIRELY of
      digit characters (0-9, no a-f) -- a syntactically valid, unquoted YAML core-schema scalar
      resolves an all-digit string to an INTEGER, not a string. `kubectl apply` therefore
      submitted a JSON payload with a numeric `stringData.rootUser` field, and the API server
      rejected it verbatim: `Error from server (BadRequest): error when creating \"STDIN\": Secret
      in version \"v1\" cannot be handled as a Secret: json: cannot unmarshal number into Go struct
      field Secret.stringData of type string` (log line 772) -> `make: *** [Makefile:167:
      cluster-up] Error 1` (line 773) -> job exit code 2. Probability check (not a guess): an
      8-byte/16-hex-char string is all-digit with probability (10/16)^16 ~= 0.00054, i.e. roughly
      1-in-1845 per `rootUser` generation -- rare enough to explain why this exact failure mode has
      NEVER appeared once across 22 prior rounds' worth of cluster-up invocations (grep of this
      entire debug file for 'stringData'/'cannot unmarshal number'/'minio-credentials' before this
      round: zero prior hits), consistent with a genuine first-time unlucky draw, not a regression
      introduced by ROUND 22's own changes. `rootPassword`/`secretKey` (32 bytes/64 hex chars each)
      are astronomically less likely to hit this (`(10/16)^64`) and were not implicated this run.
      CONFIRMED via direct source read of `scripts/minio-credentials.sh` (git blame: introduced
      commit 9c68cea, phase 02-04, unchanged since) -- a genuine, previously-latent, unrelated
      infra-layer bug, NOT touched by, caused by, or related to ROUND 22's DAG timeout/retry-budget
      changes in any way."
  criteria_adjudication: "NONE of ROUND 22's three pre-registered criteria -- (a) dbtkill/
      idempotent_reupload clearing, (b) u3/orphan clearing, (c) podkill/sweep/OOM regression
      cleanliness -- can be evaluated from this run. The E2E test suite never started; there is no
      census, no test node-IDs, no TI history, nothing downstream of `make cluster-up` to inspect.
      This run is VOID for ROUND 22's own verification purposes -- neither a pass nor a fail of the
      timeout/retry-budget fixes, simply uninformative. It must not be scored against the
      pre_registered_criteria in either direction."
  rebuild_scd2_note: "Per the checkpoint response's own instruction: test_rebuild_from_raw's
      result is separately confirmed moot for an ADDITIONAL, stronger reason this round -- the
      suite never reached that test at all (step 16, 'Run rebuild-from-raw (D-24 capstone)', shows
      status=skipped). No new information either way about the SCD2 fix. Confirmed via `git
      merge-base --is-ancestor a0cc2f5 2009065` = FALSE: this run's headSha 2009065 does NOT
      include the SCD2 fix commits (a0cc2f5/3c2c4bf come strictly AFTER 2009065 on the commit
      graph), exactly as the checkpoint response stated. The SCD2 fix still awaits its own first
      live-verification run, unaffected by anything observed in this run."
  bonus_finding_and_fix: "UNPLANNED but real: fixed the newly-discovered `scripts/minio-
      credentials.sh` quoting bug directly (single-quoted the `stringData` scalar values in
      `_create_secret`, forcing YAML's core schema to resolve them as strings regardless of
      content -- verified via a standalone PyYAML parse of the new template against a synthetic
      all-digit value, confirms `type(parsed['stringData']['rootUser']) is str`). Scope
      justification for fixing this immediately rather than deferring to a checkpoint-gated
      decision: (1) zero-risk, one-line-shape change, confined entirely to `scripts/
      minio-credentials.sh`, outside every named scope-guardrail area (dags/, packages/dataplat,
      tests/e2e budget files, CPU-starvation remediation) -- an orthogonal infra-layer fix, not a
      timeout/retry-budget decision; (2) left unfixed, this ~1-in-1845 probabilistic bug can
      recur on ANY future run at random, including the very re-verification run this round's
      outcome now requires -- fixing it removes a real, if rare, source of wasted ~3-hour CI
      cycles for something structurally unrelated to what is being tested; (3) regression-checked:
      `pytest tests/policy/test_workflow_secrets.py tests/policy/test_no_manual_kubectl_surgery.py`
      (the two test files with any awareness of this script's shape) both still pass (27/27),
      `bash -n` clean. NOT committed as part of ROUND 22's own fix set (separate commit, separate
      concern) and NOT scored against ROUND 22's own pre_registered_criteria."
  guards_and_duration: "Not applicable this run -- Kyverno/restarts/scheduler-peak/duration-
      decomposition guards all depend on the E2E suite having run; none of that instrumentation
      executed (steps 8/14's DEBUG monitor steps show status=skipped/success respectively but the
      monitor never had a live cluster to observe -- step 14 'success' reflects a no-op empty dump,
      not a real reading)."
  next_action: "DECISION CHECKPOINT returned to user: this run is void for ROUND 22 verification
      purposes due to an unrelated, now-fixed infra flake (`scripts/minio-credentials.sh` quoting
      bug). Recommended next step: since HEAD already contains BOTH ROUND 22's timeout/retry-budget
      fixes (e6f37fb/2009065) AND the separate SCD2 recompute fix (a0cc2f5/3c2c4bf, confirmed via
      `git merge-base --is-ancestor`) AND this round's own minio-credentials.sh quoting fix, a
      SINGLE fresh combined live-verification run (pushed from a new HEAD including all three) can
      verify all three simultaneously -- no new round needed unless this fresh run itself surfaces
      something new. Scope guardrails unchanged: rounds 1-22 fixes stay; runner migration/job
...
- checked: >
    ROUND 22 live-verification run 33239055603 (headSha 2009065), `gh run view --json jobs` then
    `gh api .../jobs/99065100089/logs` (saved scratchpad round22-job.log, 1,241 lines).
  found: >
    Job conclusion=failure, 06:41:00Z->06:44:24Z = 3m24s total -- died at step 7 ('Bring up the
    ephemeral kind cluster (CI profile)', `make cluster-up`), steps 9-13/15-16 (image points,
    migrations, Vault, the E2E suite itself, observability, rebuild-from-raw) all status=skipped.
    Log line 772: `Error from server (BadRequest): error when creating "STDIN": Secret in version
    "v1" cannot be handled as a Secret: json: cannot unmarshal number into Go struct field
    Secret.stringData of type string` during stage `60-minio.sh`'s `scripts/
    minio-credentials.sh ensure` call -> `make: *** [Makefile:167: cluster-up] Error 1` (line 773).
    Direct source read of `scripts/minio-credentials.sh`'s `_create_secret` helper: emits
    `stringData` values as UNQUOTED YAML scalars; `rootUser` is `_random_hex 8` (16 lowercase-hex
    chars). An unquoted all-digit (0-9, no a-f) hex string is a syntactically valid YAML integer
    under the core schema -- this run's randomly-generated `rootUser` happened to be all-digit,
    so `kubectl apply` submitted a numeric JSON `stringData.rootUser`, which the API server's Go
    types rejected. Probability check: (10/16)^16 ~= 0.00054 (~1-in-1845) per generation --
    `rootPassword`/`secretKey` (64 hex chars each) are practically immune ((10/16)^64). Grepped
    this entire debug file for 'stringData'/'cannot unmarshal number'/'minio-credentials' before
    this round: zero prior hits across 22 rounds. Fixed directly: `_create_secret` now
    single-quotes every `stringData` scalar (`printf "  %s: '%s'\n" ...`), verified via a
    standalone PyYAML parse that an all-digit value now resolves to `str`, not `int`. Regression
    check: `pytest tests/policy/test_workflow_secrets.py tests/policy/
    test_no_manual_kubectl_surgery.py` 27/27 passed (unchanged), `bash -n` clean.
  implication: >
    This run is VOID for ROUND 22's own pre_registered_criteria (a)/(b)/(c)/(d) -- the E2E suite
    never started, so dbtkill/idempotent_reupload/u3/orphan/podkill/sweep/OOM/rebuild all remain
    unobserved this run, neither confirmed nor refuted. The failure is a genuine, previously-latent,
    rare (~1-in-1845) infra-layer bug entirely unrelated to ROUND 22's DAG timeout/retry-budget
    changes, its own first occurrence in this session's 22 rounds. Now fixed, removing a real
    (if rare) source of future wasted ~3-hour CI cycles. HEAD (post-fix) contains ROUND 22's own
    fixes (e6f37fb/2009065) + the separate SCD2 fix (a0cc2f5/3c2c4bf, confirmed NOT included in
