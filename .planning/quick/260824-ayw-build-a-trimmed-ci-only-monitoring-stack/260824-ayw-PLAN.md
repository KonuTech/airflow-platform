...
<task type="auto">
  ...
  <action>
    ...
    Add a second new target `observability-verify-ci` immediately after `cluster-slice-verify`. Its recipe body MUST be one continuous shell invocation, matching `rollback`'s own established `set -e`-guarded backslash-continuation shape exactly (read it before writing this — `migrate-analytics` omits `set -e` and compensates with manual `if`/`exit 1` checks instead; `rollback`'s simpler `set -e;`-line shape is the one to mirror here) — every physical recipe line except the last ends in a literal trailing `\` continuation character, because a bare newline with no trailing `\` starts a brand-new subshell under Make (absent `.ONESHELL:`), and `ctx`/`set -e` established on an earlier line would not apply to a later one. Without `set -e`, Bash does NOT abort a semicolon-chained command list on an intermediate nonzero exit — it continues to the next statement, and the whole chain's own exit status (which becomes `make`'s exit status) is only the exit status of the LAST command, so a failing `pytest tests/e2e/observability` would be silently swallowed as long as `monitoring-teardown.sh` itself succeeds afterward, and the CI job would report green regardless of whether the observability suite passed. Write it as six physical lines: line 1 `@set -a; . helm/versions.env; set +a; \`; line 2 `set -e; \`; line 3 `ctx="kind-$$CLUSTER_NAME"; \`; line 4 `PROFILE=ci KUBECTL_CONTEXT="$$ctx" scripts/monitoring-install.sh; \`; line 5 `$(RUN_CLUSTER) pytest tests/e2e/observability -q; \`; line 6, with NO trailing `\` since it is the final command in the chain, `KUBECTL_CONTEXT="$$ctx" scripts/monitoring-teardown.sh`. `PROFILE=ci` is a literal on line 4 (not `$(PROFILE)`) since this target only ever makes sense against the single-node CI cluster shape. Header comment above the target: explains the staggering rationale (CONTEXT.md's locked CPU-contention decision — monitoring live only for this window, not the whole job); that with `set -e` in effect, teardown runs only after a successful pytest run, and the recipe aborts (leaving monitoring installed) if either the install or the pytest step itself fails — an accepted trade-off given the whole ephemeral kind cluster is destroyed at job end regardless, not a bug to fix here; and that this is about honest CI failure signal (a real regression in `tests/e2e/observability` must fail this `make` target, not be silently swallowed by a later command's own success), not about protecting `make rebuild-from-raw` from a leftover install (that later step has no `if: always()` and never runs in this failure branch anyway, per Task 1's `monitoring-teardown.sh` header comment).
