...
Layered on top of the roster (plan 10-06's Tasks 2-3): D-11's three required
anomalies -- a deterministic attribute change, a late/out-of-order correction
landing on a specific day, and a single missing customer -- plus D-06's
mass-delete/circuit-breaker-trip anomaly (an implausibly large single-day
disappearance), each an optional, independently-triggered keyword parameter
recorded on ``BackfillCorpusManifest`` so a downstream live test can assert
against known values instead of re-deriving them from the generated bytes.
...
class BackfillCorpusManifest:
    ...
    Attributes:
        ...
        mass_delete_day_index: Day index from which an entire block of
            roster members is omitted (D-06's circuit-breaker-trip fixture).
            ``None`` when not supplied.
        mass_delete_member_indices: Roster member indices omitted on
            ``mass_delete_day_index``. ``None`` when not supplied.
    """
    ...
    mass_delete_day_index: int | None = None
    mass_delete_member_indices: tuple[int, ...] | None = None
...
def generate_dated_series(  # noqa: PLR0913 -- one keyword per D-06/D-10/D-11 injected-anomaly control point
    ...
    *,
    ...
    Args:
        ...
        mass_delete_day_index: ``customers``-only (D-06). Day index from
            which every member in ``mass_delete_member_indices`` is omitted
            from that single day's file — the circuit-breaker-trip fixture.
            Requires ``mass_delete_member_indices``.
        mass_delete_member_indices: ``customers``-only (D-06). Roster member
            indices omitted on ``mass_delete_day_index``. Requires
            ``mass_delete_day_index``.
    ...
    Raises:
        ValueError: If ``dataset`` is not a known dataset name; if any
            ...
            if any of the four paired parameters (attribute-change,
            late-correction, missing-customer, mass-delete) has exactly one
            of its two halves set; or if two distinct anomalies target the
            identical (day, roster-member) pair.
    """
    ...
    _validate_customers_only_params(
        ...
        mass_delete_day_index=mass_delete_day_index,
        mass_delete_member_indices=mass_delete_member_indices,
    )
    ...
    mass_delete_set: frozenset[int] = frozenset()
    if dataset == "customers":
        ...
        mass_delete_set = (
            frozenset(mass_delete_member_indices) if mass_delete_member_indices else frozenset()
        )
        ...
        if dataset == "customers":
            lines.extend(
                _render_customer_day_lines(
                    ...
                    missing_customer_member_index=missing_customer_member_index,
                    mass_delete_day_index=mass_delete_day_index,
                    mass_delete_set=mass_delete_set,
...
def _validate_customers_only_params(  # noqa: PLR0913 -- mirrors generate_dated_series' own anomaly-parameter set
    ...
    *,
    ...
    mass_delete_day_index: int | None,
    mass_delete_member_indices: tuple[int, ...] | None,
) -> None:
    """Fail loud, never silently ignored: D-11/D-06 anomalies are customers-only."""
    if dataset == "customers":
        return
    offending = {
        "attribute_change_day_index": attribute_change_day_index,
...
def _validate_paired_params(  # noqa: PLR0913 -- mirrors generate_dated_series' own anomaly-parameter set
    *,
    ...
    consistently across all four anomalies rather than leaving three of
    them silently under-validated).
    """
...
def _check_no_anomaly_collisions(  # noqa: PLR0913 -- mirrors generate_dated_series' own anomaly-parameter set
    *,
    ...
    if mass_delete_day_index is not None and mass_delete_member_indices is not None:
        pairs.extend(
            (mass_delete_day_index, member_index, "mass_delete")
            for member_index in mass_delete_member_indices
        )
...
def _render_customer_day_lines(  # noqa: PLR0913 -- one keyword per roster-rendering context value
    *,
    ...
    missing_customer_member_index: int | None,
    mass_delete_day_index: int | None,
    mass_delete_set: frozenset[int],
    include_extra: bool,
    extra_values: tuple[str, ...],
) -> list[str]:
    """Render one day's full roster resend, D-11/D-06 overrides applied.

    Every roster member is emitted in ascending customer_id order (index
    order), unless removed by missing_customer/mass_delete for this exact
    day. The late-correction anomaly, if it targets this day, appends one
    EXTRA row after the full roster -- a real bronze delivery would arrive
    as an additional row, never a silent substitution.
    """
    ...
    for member_index, customer_id in enumerate(roster_ids):
        if (
            ...
            continue
        if day_index == mass_delete_day_index and member_index in mass_delete_set:
            continue
