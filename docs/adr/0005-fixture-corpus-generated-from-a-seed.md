...
### Scope: the whole corpus was authored in Phase 1, not the recommended subset

The research recommended a smaller Phase-1 scope — "the full 29 from §73 plus every
byte-level-hard addition plus the four decision-pinning semantic ones ≈ 50 fixtures",
with the remaining ~19 explicitly deferred. **All 69 were authored in Phase 1 instead**,
across plans 01-03 (5 seeded), 01-06 (16 byte-level), 01-07 (31 structural) and
01-08 (17 semantic).

The reason the deferral stopped being worth its cost: 01-06 predicted that the
remaining fixtures would need no generator change, and 01-07 then added 31 fixtures
with `git diff --stat tools/corpus/` empty, as did 01-08 with its 17. Once that
prediction was observed twice, the argument for deferring collapsed — a deferred
fixture is a five-line manifest entry plus one digest line, while a *missing* fixture
is a decision nobody has made yet. The semantic cases in particular (`50`, `51`, `52`,
`55`) exist to pin what a normaliser must NOT do, and their value is entirely
front-loaded: writing them after the normaliser exists means writing them to match
whatever it already does.

Two consequences follow, and both are load-bearing for later phases:

* **The `expect:` sub-schema stays permissive** (the module docstring in
  `tools/corpus/manifest.py` records this as Open Question 3's resolution). Authoring
  all 69 declarations in Phase 1 means writing expectations in a vocabulary whose terms
  are defined later — quarantine reasons (Phase 8), encoding-confidence floors, locale
  profile names, zone-transition classifications, null-token and boolean mappings. A
  strict sub-schema would have forced Phase 1 to invent that vocabulary prematurely, or
  forced a model migration in Phase 6. The permissiveness is the price of front-loading
  the decisions, and it is the right trade because the alternative is a schema that
  checks nothing anyway.
* **Phases 6 and 8 own that vocabulary, not Phase 1.** The `expect:` blocks are the
  input to Phase 6's parametrised detector loop and Phase 8's quarantine-reason
  enumeration. Phase 1 wrote down *what each fixture means*; those phases decide what
  the terms are called and enforce them.

The digest oracle cannot check any of this: an `expect:` block is not part of the
generated bytes, so a declaration can drift from its fixture with a perfectly stable
digest. That gap is closed by `tests/unit/test_corpus_byte_level_fixtures.py` (01-06),
`test_corpus_structural_fixtures.py` (01-07) and `test_corpus_semantic_fixtures.py`
(01-08), which parse the generated bytes and assert against each fixture's own
declaration — including a completeness check comparing the declared names against the
sixty-nine transcribed from README §73 and FEATURES.md §3.4.

"Complete" means complete against those two sources, **not closed**. README §73's own
wording is "grow the corpus as edge cases are discovered", and growth remains one
manifest entry plus one digest line.
...
* `tests/fixtures/corpus.yaml` — the manifest; the machine-readable specification
  (69 declarations, 70 emitted files — `62_multipart_split` emits two parts)
* `tests/fixtures/CORPUS.sha256` — the oracle, one line per emitted file
* `tools/corpus/` — the generator (plan 01-03, extended once in 01-06 and unchanged since)
* `tests/unit/test_corpus_byte_level_fixtures.py`,
  `tests/unit/test_corpus_structural_fixtures.py`,
  `tests/unit/test_corpus_semantic_fixtures.py` — the assertions the digest cannot make
