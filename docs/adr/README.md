...
|---|---|---|
| MinIO: pin the `pgsty` community fork; SeaweedFS is the named migration target | **Phase 2** | The decision is only actionable when the Helm values file exists, and its migration trigger — "the fork goes stale" — is meaningless before anything runs on it. |
